...
#### test bucketfill
    def ok_clicked(self, widget):
        ...
        #debug(('K195 And the winners are...', selected, tsSel), 1)
        self.selected_items = [el[0] for el in res1]
        debug(( self.selected_items),1)
...
############ end guides-to-path
    ...
    def tabHelp2(self, tab):
        ...
        """
        msg2 = [("pushing the button <b>Path on Guides inside selections</b> creates a path") ,
                ("you may stroke an available path with pencil and background-color"),
                ("you may opt for deleting the the active vextors") +"\n",
                ("<span foreground='blue' background='white'>You may have a selection active, then only guides INSIDE </span>"),
    ...
    def tabOne(self):
        ...
        try:
            ...
            self.ui.addRows(linesep1, self.tabs[0],0,10, row,row + 1)
            row += 1
            self.checkynGUI = gtk.CheckButton("This window always on top: y/n")
            self.ui.addRows(self.checkynGUI, self.tabs[0], 2,4, row, row + 1, 'clicked', self.call_plugin)
...
#######
            ...
            self.ui.addRows(self.newLabel, self.tabs[2], 0, 5, row, row + 1 )
            row +=1
            self.ellipse_or_rectangle = self.ui.addRows(self.ui.makeCombo([['ellipse',2],['rectangle',1]]), self.tabs[2], 0, 1, row, row + 1)

            self.ui.addRows(gtk.Label("set width"), self.tabs[2], 1, 2, row, row + 1)
            ...
            r1 = self.ui.addRows(self.widthSpinButton, self.tabs[2], 2, 3, row, row+1,"activate", self.evalSpin)
            self.ui.addRows(gtk.Label("set height"), self.tabs[2], 3, 4, row, row + 1)
            ...
            r2 = self.ui.addRows(self.heightSpinButton, self.tabs[2], 4, 5, row, row + 1,"activate", self.evalSpin)

            row += 1
            self.ui.addRows(gtk.Label("fill selections?!"), self.tabs[2], 0, 1, row, row + 1)
            self.filltype = self.ui.addRows(self.ui.makeCombo([['no',0],['fg color',1],['centercolor',2],['pattern',3],['kacheln',4]]), self.tabs[2], 1, 2, row, row + 1)
            self.ui.addRows(gtk.Label("opacity fill-bucket?!"), self.tabs[2], 2, 3, row, row + 1)
            self.opacityforbucket = gtk.SpinButton(gtk.Adjustment(100, 0, 100, 5), 0.0, 0)
            self.widthSpinButton.set_name('opacityforbucket')
            r1 = self.ui.addRows(self.opacityforbucket, self.tabs[2], 3, 4, row, row+1,"activate", self.evalSpin)
            self.ui.addRows(gtk.Label("random col. or pat.?!"), self.tabs[2], 4, 5, row, row + 1)
            self.pattern_checkyn = gtk.CheckButton("y/n")
            self.ui.addRows(self.pattern_checkyn, self.tabs[2], 5, 6, row, row + 1, 'clicked', self.call_plugin)
            row +=1
            
            ellipseButton = gtk.Button("selection(s) at intersection(s)")
            self.ui.addRows(ellipseButton, self.tabs[2], 0, 7  , row, row + 1, 'clicked', self.createellipses)
            row += 1
            #self.ui.addRows(gtk.Label("New layer"), self.tabs[2], 0, 1, row, row + 1)
            self.new_layer = gtk.Button("create a new RGBA layer (at top!). The top-layer is always used")
            self.new_layer.set_name("create_new_layer")
            self.ui.addRows(self.new_layer, self.tabs[2], 0, 2 , row, row + 1, 'clicked', self.call_plugin)
    ...
    def showDialog(self):
        ...
        for n in range(0, len(self.tabTits)):
            if n < 4:
                ...
                mydata = ['Arrow', 'Axis of symmetry', 'Binoculars',
                ...
                debug(("L1088 just created treeview =",self.treeview),1)
                self.treeview.set_rules_hint(gtk.TRUE)    
                column = gtk.TreeViewColumn('Your choice(s)', gtk.CellRendererText(),
                            text=COLUMN_TEXT)
                self.treeview.append_column(column)
                self.treeview.get_selection().set_mode(gtk.SELECTION_MULTIPLE)
                selected = ["peter was here"]
                #frame = gtk.Frame("to choose several ... at once")
