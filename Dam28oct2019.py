...
bordDone = True
sys.path.append(gimp.directory + "/dammen")
#print(sys.path[-1])
...
class Damdevellop28apr0(object):
    ...
    def __init__(self, runmode, img):
        ...
        if runmode == RUN_INTERACTIVE:
            self.showDialog()
            debug("L754 " .format(dir(self.showDialog())))
    ...
    def insertTheseStones(self, stoneList, color, damsList):
        ...
        self.inititializeTheBord()
        if color == 'black':
            use_pattern = 'dammenblack'
        else:
            use_pattern = 'dammenwhite'
    ...
    def makeScreenshot(self, widget):
        ...
        for file in os.listdir(gimp.directory + '/dammen/steps/'):
            if file.startswith("dammenstep"):
                ...
                nrNow += 1
        result = self.saveThisStep(gimp.directory + '/dammen/steps/pictures', str(nrNow).zfill(2))
    ...
    def changed_cb(self, combobox):
        ...
        if index > -1: #and stoneBoxDBG:
            ...
            #PKHG>04-22 17:25 TODO
            fdebug(("L994 clicked is where color = ", where, self.activeColorValue))
    ...
    def showDialog(self):
        ...
        dialog.set_keep_above(True);
        dialog.set_resizable(True) #PKHg>29oct test1
        ...
        colorTryMove = map.alloc_color("#3f3") #PKHG>was 3f3 (groen?)
        ...
        colorScreenshot = map.alloc_color("#a80") #was 6ff
        #copy the current style and replace the background
        ...
        colorMoveMap = movesMap.alloc_color("#00f") #was 2e2
...
#next two def's to addd checkboxes
    ...
    def moveStone(self, fromTo):
        ...
        if saveQuestion:
            debug(("L1555 save picture by moveStone"), 1)
...
"('L2708 RETURN oldies', [], [])"
        ...
        if jumpList:
            ...
            #PKHG>OK
            #PKHG>OK04-21 f
            debug(("L2791 04-20 09:45  no capture jump"), 0 )
...
class Damdevellop28apr0StarterClass(gimpplugin.plugin):
    def start(self):
        print("\n-----19oct2019-------------Damdevellop28apr0StarterClass called\n")
        gimp.main(self.init, self.quit, self.query, self._run)
    def init(self):
        pass
    def quit(self):
        print("\n---------PKHG---------Damdevellop28apr0StarterClass ended\n")
        pass
    def query(self):
        cright = "PeterPKHG "
        date = "1feb2017 old date, now 29oct2019"
        ...
        gimp.install_procedure("damdevellop_DamNew", plug_descr, plug_descr, "PeterPKHG", cright, date,
                               "<Image>/GTK/Dam28oct2019", "RGB*, GRAY*", #
                               PLUGIN, plug_params,[]) #
